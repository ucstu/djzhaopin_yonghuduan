    ...
    <div class="password">
        ...
        <div>
            <el-form ref="ruleFormRef" :model="ruleForm" status-icon :rules="rules" label-width="120px"
                ...
                <el-form-item>
                    <el-button type="primary" @click="updateForm(ruleFormRef)">保存</el-button>
                </el-form-item>
            </el-form>
        </div>   </div>
</template>

<script setup lang="ts">import { putAccounts, putAcocuntsAccountid } from '@/services/services';
...
const updateForm = (formEl: FormInstance | undefined) => {
    ...
    formEl.validate((valid) => {
        if (valid) {
            putAcocuntsAccountid(store.state.accountInfo.accountId, ruleForm).then(res => {
                console.log(res);
                if (res.status === 200) {
                    ruleForm.password = ''
                    ruleForm.checkPass = ''
...
<style scoped lang="scss">.password {
    margin: 40px 0;
